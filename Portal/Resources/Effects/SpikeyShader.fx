...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	float3 basePoint, top, left, right;
	...
	float3 baseNormal = normalize((vertices[0].Normal + vertices[1].Normal + vertices[2].Normal) / 3);
	//Step 3. Calculate The Spike's Top Position
	top = basePoint + (8 * baseNormal);
	//Step 4. Calculate The Left And Right Positions
	float3 spikeDir = normalize(vertices[2].Position - vertices[0].Position) * 0.25f;

	left = basePoint - spikeDir;
	right = basePoint + spikeDir;

	//Step 5. Calculate The Normal of the spike
	float3 spikeNormal = normalize(cross((left - top), (right - top)));

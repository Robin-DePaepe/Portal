...
HRESULT OverlordGame::Run(HINSTANCE hInstance)
{
	...
	const float fixedTimeStep{ 1.f / DesiredFPS };
	float accumTime{ 0 }, accumTime2{ 0 };
	int frameCount{ 0 };

	GameTime* pTime{ new GameTime() };
	pTime->Reset();
	pTime->Start();
	...
	while (msg.message != WM_QUIT)
	{
		...
		accumTime += pTime->GetElapsed();
		accumTime2 += pTime->GetElapsed();

		if (accumTime >= fixedTimeStep)
		{
			accumTime -= fixedTimeStep;
			GameLoop();
			++frameCount;
		}
		if (accumTime2 >= 1.f)

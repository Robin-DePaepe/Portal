...
void RigidBodyComponent::SetCollisionIgnoreGroups(CollisionGroupFlag ignoreGroups, ColliderComponent* pCollider)
{
	physx::PxFilterData temp{ physx::PxFilterData(static_cast<UINT32>(CollisionGroupFlag::Group0), 0, 0, 0) };
	temp.word1= static_cast<UINT32>(ignoreGroups);

	pCollider->GetShape()->setSimulationFilterData(temp);
}

void RigidBodyComponent::SetCollisionGroup(CollisionGroupFlag group, ColliderComponent* pCollider)
{
	physx::PxFilterData temp{ physx::PxFilterData(static_cast<UINT32>(CollisionGroupFlag::Group0), 0, 0, 0) };
	temp.word1 = static_cast<UINT32>(group);

	pCollider->GetShape()->setSimulationFilterData(temp);
	pCollider->GetShape()->setQueryFilterData(temp);
...
void RigidBodyComponent::Scale(float scale)
{
	physx::PxScaleRigidActor(*m_pActor, scale);
}

...
public:
	...
	void SetPickup(bool value) {	m_IsPickup = value;	}
	bool IsPickup() { return m_IsPickup; }

	void SetPortalWall();
	bool IsPortalWall() { return m_IsPortalWall; }
	...
	std::vector<T*> GetComponents(bool searchChildren = false)
	{
		...
		if (searchChildren)
		{
			for (auto* child : m_pChildren)
			{
				...
				for (auto* pChildComp : childComponents)
					components.push_back(static_cast<T*>(pChildComp));
			}
		}
...
private:
	...
	bool m_IsInitialized, m_IsActive, m_IsPickup, m_IsPortalWall;

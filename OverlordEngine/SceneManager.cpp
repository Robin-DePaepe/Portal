...
SceneManager::SceneManager() :
	...
	m_pRemovedScenes(std::vector<GameScene*>()),
...
SceneManager::~SceneManager()
{
	...
	for (GameScene* scene : m_pRemovedScenes)
	{
		SafeDelete(scene);
	}
}

bool SceneManager::IsScenePresent(const std::wstring& sceneName)
{
...
void SceneManager::AddGameScene(GameScene* pScene)
{
	...
	if (it == m_pScenes.end())
	{
		...
		if (m_ActiveScene == nullptr && m_NewActiveScene == nullptr)
		{
			...
			Logger::LogInfo(L"Scene: '" + m_NewActiveScene->m_SceneName + L"' activated!");
		}
	}
}
...
void SceneManager::RemoveGameScene(const std::wstring& sceneName)
{
	...
	if (it != m_pScenes.end())	RemoveGameScene(*it);
}
